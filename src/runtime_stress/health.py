...
# cap bit, the other says the host memory cap did, and E2 must tell them apart.
_CUDA_OOM_MARKERS = (
    "CUDA out of memory",
...
# droidslam's E2 VRAM 75% runs each log it -- asking 7143047552 against a
...
def oom_attribution(run_dir: Path, log: Optional[str]) -> Dict[str, Any]:
    ...
    destroyed runs, so E2 would otherwise be inferring "which axis killed it"
...
def fatal_error_evidence(oom: Dict[str, Any], log: Optional[str]) -> List[str]:
    ...
    # as missing data. E4NW hit this on four s3pogs rungs, where the trajectory
...
def verdict_for_run(run_dir, system: str,
    ...
    #
    # This is not a theoretical gap. E1's offline s3pogs cell printed 1.022, the
    ...
    # Anchoring to clean pose count was tried and re-scored against E1's 144
    ...
    # severity; calling that degradation flagged 42 of E1's 120 runs wrongly.
