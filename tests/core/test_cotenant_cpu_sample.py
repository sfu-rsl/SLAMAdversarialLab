...
That gap matters for what E5 is allowed to claim. E4's GPU generator is fenced to
2 cores and 8 GB, so GPU-only contention holds by construction. E5's co-tenant is
not fenced at all, so the same claim rests on the empirical control that the
