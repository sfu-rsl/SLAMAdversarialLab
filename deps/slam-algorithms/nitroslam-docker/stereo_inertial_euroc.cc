...
{   
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        // loop run identical processing. IMU accumulation is timestamp-driven
        // (running first_imu[seq]), so a surviving frame after dropped frames
        // still receives every IMU sample since the last survivor.
        ...
        if(dl.active())
        {
            ...
            // passed because tracking was too slow. Dataset-rate pacing is
            // skipped here (dl.next() does the pacing) to avoid pacing twice.
