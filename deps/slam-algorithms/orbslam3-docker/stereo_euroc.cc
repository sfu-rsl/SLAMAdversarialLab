...
#include<System.h>
// === SAL deadline: real-time frame-dropping. No-op unless SAL_DEADLINE_FPS is
// set; copied beside this file at build time. ===
...
{  
    ...
    for (seq = 0; seq<num_seq; seq++)
    {
        ...
        // loop run identical processing. Vision-only (no IMU): a surviving frame
        // after dropped frames carries its own original timestamp, so the SLAM
        // sees a sparser stream with correct timing and nothing fills the gap.
        ...
        sal::DeadlineIterator dl(nImages[seq]);
        if(dl.active())
        {
            // dl.next() paces to SAL_DEADLINE_FPS and drops frames whose deadline
            // passed because tracking was too slow. The lambda's own dataset-rate
            // sleep is skipped implicitly (dl.next() does the pacing).
