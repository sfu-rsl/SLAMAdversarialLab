...
// === SAL deadline: begin ===
// Generic SAL real-time deadline iterator (single source of truth:
// src/runtime_stress/deadline_iterator.h, copied in beside this file at
// image build time). Inactive unless SAL_DEADLINE_FPS is set in the env.
...
void  DatasetReader::processing() {
  ...
  // === SAL deadline: begin ===
  // Frame indices are cam0-keyed (== counter_), matching the sampled frame
  // list SAL staged into cam0/data.csv. Function-local so its destructor
  // writes the drop log when the stream ends by any path. IMU (and GPS)
  // delivery below stays unconditional: only camera frames are dropped,
  // which is what a loaded real-time VIO does.
  sal::DeadlineIterator salDeadline(
      allImageNames_.empty() ? 0 : static_cast<int>(allImageNames_.at(0).size()));
  // === SAL deadline: end ===
  ...
  while (streaming_) {
    ...
    // === SAL deadline: begin ===
    // Deliver only frames that met their wall-clock deadline; dropped frames
    // skip the (blocking) SLAM callback entirely. should_deliver() paces to
    // the target FPS, so the consumer's own slowness (backpressure through
    // the blocking callback) is what causes drops. counter_ still advances
    // for every dataset frame (it tracks dataset progress, not deliveries).
    // Gate the deadline query, delivery, and survivor counter together on
    // cam0 presence so should_deliver() always sees a strictly monotone
    // counter_ (each k queried exactly once). Matches XDatasetReader. For the
    // only supported config (EuRoC stereo) cam0 is always present, so this is
    // a robustness hardening, not a behavior change.
    if(images.count(0) && !images.at(0).empty()) {
      if (salDeadline.should_deliver(static_cast<int>(counter_))) {
