...
void  XDatasetReader::processing() {
  ...
  // delivery stays unconditional: only camera frames are dropped, which is
  // what a loaded real-time VIO does.
  ...
  while (streaming_) {
    ...
    if(images.size() == numCameras) {
      // === SAL deadline: begin ===
      // Deliver only frames that met their wall-clock deadline; dropped
      // frames skip BOTH the SLAM callback and the depth-network callback
      // (a dropped camera frame produces no depth either). counter_ still
      // advances for every dataset frame (dataset progress, not deliveries).
